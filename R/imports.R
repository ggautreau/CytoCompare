...
import.CELL <- function(file,
    ...
    data     <- read.table(file,header=TRUE,sep="\t",row.names=1,stringsAsFactors=FALSE,check.names=FALSE)
...
#' @param trans.para a character named list containing parameters for the transformation. Please refer to the details section for more details
#' @param trans.exclude a character vector containing the marker names for which no transformation will be applied on
#'
#' @return a S4 object of class CELL or CLUSTER
#'
#' @export
import.FCS <- function(path,
                        ...
                        exclude       = NULL,
                        trans         = "arcsinh",
                        trans.para    = list(arcsinh.scale=5),
                        trans.exclude = NULL){
    ...
    for(i in 1:length(path)){
        ...
        if(trans=="arcsinh"){
            ...
            for(marker in markers){
                data[,marker] <- log(data[,marker] + trans.para$log.shift,trans.para$log.base)
            }
        }else if(trans!="none"){
...
#'
#' @description Imports one or several cell cluster profiles from a tab separated file into a CLUSTER object.
#'
...
import.SPADE <- function(path,
    ...
    if(zip){
        ...
        unzip(path,exdir = extract.folder)
        message("done")
        path <- extract.folder
    }
    ...
    cluster@graph.layout       <- as.matrix(read.table(paste0(path,"/layout.table")))
