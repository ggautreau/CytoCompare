...
setMethod("plot",c("DENSITY","missing"),
    function(object1,object2,return.gg=FALSE){
        ...
        bin.neg <- seq(from=object1@bin.interval[1],to=0,by=object1@bin.width)
        bin.pos <- seq(from=object1@bin.width,to=object1@bin.interval[2],by=object1@bin.width)
        bin     <- c(bin.neg,bin.pos)
        ...
        frame$smooth <- as.vector(smooth(frame$value))
...
setMethod("plot",c("RES","missing"),
    function(object1,return.gg=FALSE,...){
        ...
        for(i in 1:object1@comparisons.nb){
            ...
            frame <- data.frame(markers     = object1[i]@markers,
                                marker.measures = as.vector(unlist(object1[i]@marker.measures)),
            ...
            values            <- as.vector(unlist(object1[i]@marker.successes))
            if(all(values))
                values = "green"
            else if(all(!values))
                values = "red"
            ...
            ymin <- min(frame$marker.measures)
            ymax <- max(max(frame$marker.measures),1)
            
            plots[[plots.count]] <- ggplot2::ggplot(frame,ggplot2::aes_string(x="markers",y="marker.measures",fill="success")) +
                ggplot2::ggtitle(paste(object1[i]@comparisons[1,]$profile1," vs. ",object1[i]@comparisons[1,]$profile2,"\n combined measure = ",format(object1[i]@comparisons[1,]$measure,nsmall=4,digit=4),"\n combined p-value = ",format(object1[i]@comparisons[1,]$pvalue,nsmall=4,digit=4))) +
...
ggplot.viSNEmap <- function(profile,default.min=-3){
    ...
    layout$cols      <- densCols(layout$x,layout$y,colramp = colramp)
...
#'
#' @details In such representation, each dot corresponds to a cell profile and dot are plotted in a 2-dimensional space corresponding to the marker expressions. 
#'
...
#'
#' @export
dheatmap <- function(cluster,return.gg=FALSE){
    plots <- list()
    ...
    for(i in 1:cluster@profiles.nb){
        ...
        plots[[i]] <- ggplot2::ggplot(den.value)+
            ...
            ggplot2::scale_color_gradientn(colours=c("black","darkgoldenrod3")) +
            ggplot2::xlab("markers")+
            ggplot2::ylab("marker expression") +
            ggplot2::theme(axis.text.x = ggplot2::element_text(angle=290, hjust=0, vjust=1),
                legend.position="bottom",
